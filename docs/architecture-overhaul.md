...
8. **Persistence safety** — split into two sub-phases:
   - **8a. Persistence chokepoint.** Move all `localStorage` access into `src/persistence/<blob>/` modules (one per blob: `settings`, `progress`, `customVoicings`, `customPresets`, `preferredVoicings`). Add Zod (or equivalent) schemas; live types are `z.infer<typeof schema>`, never hand-written interfaces. ESLint rule `no-restricted-globals` (or `no-restricted-syntax`) bans `localStorage`/`sessionStorage` outside the persistence module. Refactor `useSettings`, `useProgress`, `useCustomVoicings`, `useCustomPresets`, and the `preferredVoicings` slot in `AppState` to load/save via the new modules. Runtime parse on load — malformed data falls back to defaults with a console warning; never writes garbage back. No migration plumbing yet — the schema is just "v1" and there's nothing to migrate from.
   - **8b. Migration plumbing.** Stamp `{ version, data }` on every blob. Per blob: `schemas/v1.ts`, `schemas/v2.ts`, …; a `migrate.ts` chain `vK → v{K+1}`; tests/fixtures/`<blob>/v{K}.json` (append-only corpus). Pre-commit hook check: existing `schemas/v*.ts` files are append-only (only allowed change is creation of `v{N+1}.ts`). Chain-integrity unit tests: `LATEST_VERSION` matches `max(version files in dir)`; every gap has a migration step; one fixture per version `< LATEST_VERSION`; full chain produces parseable `vN` output; idempotency; round-trip. Loader returns `{ kind: 'ok' | 'defaulted' | 'quarantined', data }` — downgrades and parse failures fail closed (defaults, no writeback), so re-upgrading restores the user's data. Also: retire any imperative navigation state still hiding in hooks (Phases 5–7 should have removed it all; audit and delete leftovers).
